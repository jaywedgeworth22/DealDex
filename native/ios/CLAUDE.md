...
**TestFlight ship:** `.github/workflows/ios-ship.yml` on `[self-hosted, macOS, ARM64, xcode26]`.  Wrapper `scripts/ios-ship-testflight.sh` prefers in-repo `scripts/ios-fleet/` (fleet key `dealdex`, bundle `net.dealdex`) and falls back to `/Users/jay/apps/ios-fleet`.  Cron is gated by `scripts/ios-scheduled-ship-gate.sh` so web-only commits do not ship.  Secrets stay in `~/.secrets/appstore-connect.env` on the Mac.  Do not switch this job to `macos-latest` — that runner has no secrets and no fleet path (every #167 ship failed in ~14s).
