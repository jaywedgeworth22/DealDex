...
**Version regimen (fleet):** `MARKETING_VERSION` / `CFBundleShortVersionString` is `1.0.<seq>` (+1 every rebuild).  `CURRENT_PROJECT_VERSION` / `CFBundleVersion` is UTC `YYYYMMDDHHMM` when the build was cut.  App Store Connect shows `1.0.2 (202608230250)`, not `1.0 (1)`.  `project.yml` records what last shipped; `ios-fleet/ship-testflight.sh` resolves the next pair on the Mac.  After a ship, sync `project.yml` + regenerate `project.pbxproj` with `xcodegen generate`.  Canonical: `/Users/jay/apps/ios-fleet/README.md`.
