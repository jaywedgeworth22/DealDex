...
## In Progress
- **2026-09-04 — GROK — IN PROGRESS — Sentry Performance child spans on Nitro/API scan hops (board `9fb9cccafb9c40b889466516a18e8dd5`, branch `grok/sentry-scan-hop-spans`, worktree `~/apps/dealdex-grok-sentry-hop-spans`).**  Spans: `scan.ebay`, `scan.mercari`, `scan.match`, `scan.enrich`, `scan.cache.hit`, `scan.cache.miss`.  Datadog stays `web.request` only.  Extra-ship no.  Local gates green (lint 0 errors, typecheck, 217 tests, build).  Rollout: `docs/rollouts/2026-09-04-sentry-scan-hop-spans.md`.
