...
## In Progress
- **2026-09-01 - GROK - IN_PROGRESS - Cap automatic Vercel deploys to one production build per hour (branch `grok/vercel-hourly-cap`, worktree `~/apps/dealdex-grok-hourly`).**  Board `9051c3ac`.  Script `vercel-ignore-hourly.sh`.  `VERCEL_FORCE_DEPLOY=1` still ships immediately.
