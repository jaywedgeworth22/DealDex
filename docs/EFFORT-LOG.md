...
## In Progress
- **2026-09-12 — CLAUDE — IN PR — Fix TestFlight ship spam: persist ship-state across ephemeral macos-latest runners (branch `claude/investigate-testflight-cadence`, worktree `~/apps/dealdex-claude`).**  Owner-reported: TestFlight updates constantly with no one shipping.  Root cause confirmed in 5 separate run logs: `ios-ship.yml`'s `~/.cache/ios-fleet/last-ship-dealdex.txt` (read by both the scheduled-ship gate and `ship-testflight.sh`'s own 1-hour min-interval/same-HEAD gate) never survives the ephemeral runner, so every tick sees "no prior ship" and ships.  Fixed with `actions/cache/restore` + `actions/cache/save` (v4.3.0 pinned), unique `github.run_id` key + prefix `restore-keys` since cache keys are immutable and this state must change every run.  Board `30abb003`.  `actionlint` clean.  Rollout: `docs/rollouts/2026-09-12-ios-ship-state-cache.md`.
- **2026-09-07 — AG — IN PROGRESS — Fix Vercel auto-deploy rate-limit query (&state=READY) (branch `ag/vercel-ignore-state-ready`, worktree `~/apps/dealdex-ag-vercel-fix`).**
- **2026-09-04 — GROK — IN PR #284 — Sentry max-features: iOS profiling + Session Replay + Android native (board `af1ab6e9`, branch `grok/sentry-max-features`, worktree `~/apps/dealdex-grok-sentry-max`).**  Android ENABLE (masked Replay 10%/100% error, profiling 0.1).  Rollout: `docs/rollouts/2026-09-04-sentry-max-features.md`.
