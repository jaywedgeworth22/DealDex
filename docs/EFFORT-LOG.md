...
## In Progress
- **2026-09-13 — AG — IN PROGRESS — Make Sentry bug reporter subtle (autoInject false + footer/error trigger) (board `4c56f2de`, branch `ag/sentry-subtle-feedback`, worktree `~/apps/dealdex-ag-sentry-subtle`).**  Set `autoInject: false` in `src/lib/observability/sentry.ts` to eliminate floating button.  Export `openSentryFeedback()` helper and add subtle trigger link to Shell footer and AppErrorComponent.
- **2026-09-07 — AG — COMPLETED — Fix Vercel auto-deploy rate-limit query (&state=READY) (branch `ag/vercel-ignore-state-ready`, worktree `~/apps/dealdex-ag-vercel-fix`).**
