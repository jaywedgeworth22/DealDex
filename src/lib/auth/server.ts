 ...
 * on this app's own origin. Sign-in uses Better Auth **socialProviders** (Google,
 * Apple, X) with this app's own OAuth client secrets — not the old Grok/xAI
 * hosted broker (`GROK_AUTH_*` is unused).
 *
 * Tri-mode:
 *   - Deployed: `GOOGLE_*` / `APPLE_*` / `TWITTER_*` (or `X_*`) + `BETTER_AUTH_URL`
 *     + `DATABASE_URL`, so real social auth is persisted in Postgres.
...
export const authConfigured = !authDisabled && socialEnabled;
...
export const auth = betterAuth({
  ...
  account: {
    ...
    accountLinking: {
      enabled: true,
      trustedProviders: SOCIAL_PROVIDERS.map((p) => p.id),
