...
export async function fetchJustTcg(card: TcgCard, key: string): Promise<ValuationQuote | null> {
  ...
  if (!res.ok) return { source: "justtcg", label: "JustTCG", usd: null, note: `JustTCG ${res.status}`, family: "listed", weight: 0.16 };
  const json = (await res.json()) as { data?: unknown[] };
  const rows = Array.isArray(json.data) ? json.data : Array.isArray(json) ? (json as unknown[]) : [];
  const usd = rows.length ? walkMoney(rows[0], ["marketPrice", "market", "price", "avgPrice", "nm"]) : null;
  return {
    source: "justtcg",
...
export async function fetchPriceChartingApi(card: TcgCard, token: string): Promise<ValuationQuote[]> {
  const q = encodeURIComponent(`${card.name} ${card.setName} pokemon`);
  const res = await fetch(`https://www.pricecharting.com/api/product?t=${encodeURIComponent(token)}&q=${q}`, {
  ...
  const psa10 = cents(product["new-price"] ?? product["psa-10-price"] ?? product["bgs-10-price"] ?? product["manual-only-price"]);
  const href =
...
export async function fetchPokemonTcgIo(card: TcgCard, key: string): Promise<ValuationQuote[]> {
  ...
  const tcg = row?.tcgplayer as { prices?: Record<string, { market?: number }>; url?: string } | undefined;
  const cm = row?.cardmarket as { prices?: { averageSellPrice?: number; trendPrice?: number }; url?: string } | undefined;
  const market =
...
export async function fetchKeyedQuotes(card: TcgCard, keys: DeskKeys): Promise<ValuationQuote[]> {
  ...
  if (keys.pricecharting) jobs.push(fetchPriceChartingApi(card, keys.pricecharting).catch(() => []));
...
export async function pingDeskKey(id: keyof DeskKeys, key: string): Promise<{ ok: boolean; message: string }> {
  try {
    if (id === "justtcg") {
    ...
    if (id === "pricecharting") {
      ...
      if (res.ok && json.status !== "error") return { ok: true, message: "PriceCharting accepted the token." };
